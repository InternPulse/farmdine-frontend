...
import leftarrow from "../assets/arrow-right-bold.png"
...
const eventData = [
	{
		id: 1,
		title: "World's Farmers Day",
		description:
			"World Farmers' Day, also known as National Farmers' Day, is celebrated on December 23rd every year. It is a day to recognize and appreciate the contributions of farmers to our society. Farmers play a vital role in feeding our communities, preserving natural resources, and supporting local economies.",
	},
	{
		id: 2,
		title: "Get The Best Farm Produce",
		description:
			"Fresh, seasonal produce delivered on your schedule. Save time and enjoy the best nature has to offer. Order today!",
	},
	{
		id: 3,
		title: "Experience the difference farm-fresh makes",
		description:
			"Elevate your meals with the freshest, highest-quality farm produce. Locally sourced fruits and vegetables bursting with flavor and packed with nutrients.",
	},
]
...
const Events = () => {
	...
	const handlePrev = () => {
		setCurrentIndex(
			(prevIndex: number) =>
				(prevIndex - 1 + eventData.length) % eventData.length
		)
		setAutoSlide(false) // Stop auto sliding on manual interaction
	}
	...
	useEffect(() => {
		...
		if (autoSlide) {
			interval = setInterval(() => {
				setCurrentIndex(
					(prevIndex: number) => (prevIndex + 1) % eventData.length
				)
	...
	return (
		<div className='flex justify-center items-center flex-col ml-3 mt-20'>
			...
			<p className='text-center mt-4 text-xl md:text-3xl px-4'>
				Farming events unite the agricultural community{" "}
				<br className='hidden md:block' /> to celebrate, share, and innovate.
			</p>

			<div className='relative w-full md:w-[800px] lg:w-[1050px] my-10 h-96 md:h-screen text-white overflow-hidden rounded-lg'>
				<img
					src='../images/eventsbg.jpg'
					alt='Event background image'
					className='absolute inset-0 object-cover w-full h-full z-0'
				/>

				<div className='text-center p-6 ml-32 transition-transform duration-500 ease-in-out transform z-10'>
				...
				>
					<img
						src={leftarrow}
						alt='arrow right'
						className='w-4 h-4 md:w-20 md:h-20 transform: rotate-180'
					/>
				</button>
				<button
					onClick={handleNext}
